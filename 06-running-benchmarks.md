...
oc create configmap benchmark-data -n llmd-test \
...
oc wait --for=condition=ready pod -l serving.kserve.io/inferenceservice=qwen-vllm \
  -n llmd-test --timeout=300s
```

### Run Baseline Benchmark

```bash
export VLLM_TARGET="http://qwen-vllm-lb.llmd-test.svc.cluster.local:8000"
...
```bash
export LLMD_TARGET="http://openshift-ai-inference-openshift-default.openshift-ingress.svc.cluster.local/llmd-test/qwen"
